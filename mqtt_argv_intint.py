...
def updatePID(strJSON):
    ...
    try: 
        ...
        if "lLimit" in data:
            lLimit_input = float(data["lLimit"])
            pid.output_limits = (lLimit_input, 100)

        if "Lmax" in data:
            Lmax_input = float(data["Lmax"])
...
class INTEG:
    ...
    def __init__(self,output,T,delayTime,discTime):
        self.output = output
        self.T = T
        self.delayTime = delayTime
        self.discTime = discTime
...
if __name__ == '__main__':
    ...
    plant = INTEG(watertemp,T1_AP2, delayTime_AP2,discTime_AP2)
    
    disturbance = INTEG(output_AP1, T_AP1, delayTime_AP1, discTime_AP1)
